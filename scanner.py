...
alphanum_set = {'A', 'a', 'B', 'b', 'C', 'c', 'D', 'd', 'E', 'e', 'F', 'f', 'G', 'g', 'H', 'h', 'I', 'i', 'J', 'j', 'K',
                ...
                'V', 'v', 'W', 'w', 'X', 'x', 'Y', 'y', 'Z', 'z'}.union(num_set)
...
def scan_for_id_keyword(seq: str, f) -> (str, bool, str):
    return seq
...
class TokenType(Enum):
    ...
    ID = (2, alphanum_set, alphanum_set + num_set, scan_for_id_keyword),
    KEYWORD = (3, alphanum_set, keywords_set, scan_for_id_keyword),
...
def get_next_token(file) -> (bool, Enum, str):
    ...
    for token_type in TokenType:
        ...
        if c in initial_set:
            ...
            read, accepted, seq, _remained_char = scan_func(c, file)
            while read:
                _cur_seq += c
                if accepted:
